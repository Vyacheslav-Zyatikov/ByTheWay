...
export default function PaymentPage() {
  ...
  const addOrders = () => {
    ...
    if (sessionId) {
      ...
      sessionData.append('session_id', sessionId);
      sessionData.append('phone', phone);
  
      axios.post(`../api/users/${sessionId}`, sessionData)
      .then((res) => {
    ...
    <Box className="restaurant">
      <Container sx={{ mt: "64px", mb: "48px", px: "0px !important" }} maxWidth="xl">
        ...
        <Box sx={{mt: "30px", display: "flex", flexDirection: "column", alignItems: "center"}}>
            ...
            <Typography mt={1} variant="h6" align="center" color="#1976D2" paragraph>Укажите телефон. Он может понадобиться для уточнения деталей заказа и времени приготовления</Typography>
            <TextField value={phone} onChange={(e) => setPhone(e.target.value)} label="Номер телефона" variant="outlined" sx={{mb: "20px", width: "400px"}}/>
        </Box>
